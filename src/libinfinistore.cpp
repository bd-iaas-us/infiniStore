            ...
            while ((num_completions = ibv_poll_cq(conn->cq, 10, wc)) && num_completions > 0) {
                for (int i = 0; i < num_completions; i++) {
                    ...
                    if (wc[i].opcode == IBV_WC_SEND) {  // read cache/allocate msg: request sent
                        ...
                        // get all remote addresses and send to server for commiting keys
                        // auto *info = reinterpret_cast<std::tuple<remote_block_t *, size_t>
                        // *>(wc[i].wr_id); auto *p_remote_blocks = std::get<0>(*info); auto
                        // remote_blocks_len = std::get<1>(*info);
